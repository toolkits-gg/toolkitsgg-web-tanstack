import type { BaseRemnant2Item } from "#/games/remnant2/core/types";

type Remnant2ModItem = BaseRemnant2Item & {};

const MODS: Remnant2ModItem[] = [
	{
		category: "MOD",
		name: "Abrasive Rounds",
		internalSlug: "Mod_AbrasiveRounds_C",
		imageUrl: "items/mods/abrasiverounds.png",
		id: "h9k5pd",
		dlc: "DLC2",
		description: [
			"Imbues ammunition with BLEED and increases Lifesteal by 5% for 20s. Shots also apply BLEEDING, dealing 200 -> 600 damage over 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Abrasive_Rounds`,
		location: { world: "Yaesha", dungeon: ["Earthen Coliseum"] },
	},
	{
		category: "MOD",
		name: "77 79 68",
		internalSlug: "Mod_Polyactuator_C",
		imageUrl: "items/mods/polygun.png",
		dlc: "DLC2",
		id: "777968",
		description: ["TOGGLE COMBAT FIRING MODESTOGGLE COMBAT FIRING MODES"],
		wikiUrl: `https://remnant2.wiki.gg/Polygun`,
		linkedItems: {
			weapon: {
				name: "Polygun",
			},
		},
		location: { world: "Labyrinth", dungeon: ["Labyrinth"] },
	},
	{
		category: "MOD",
		name: "Accelerator",
		internalSlug: undefined,
		imageUrl: "items/mods/atomsmasher.png",
		id: "h9k5pl",
		dlc: "BASE",
		description: [
			"Charge Melee Attacks increase the speed of all Melee Attacks by 10% for 5s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Atom_Smasher`,
		linkedItems: {
			weapon: {
				name: "Atom Smasher",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Aftermath",
		internalSlug: "Mod_Aftermath_C",
		imageUrl: "items/mods/aftermath.png",
		id: "vi86jx",
		dlc: "DLC3",
		description: [
			"On Relic Use, Redeemer stores the wielder's equipped Relic effect with the weapon as a launchable payload. Upon firing, the payload bursts across 4m when contacting a human ally, or lays dormant up to 15s until an ally gets close. Max 1 stored Relic which cannot be modified.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Redeemer`,
		linkedItems: {
			weapon: {
				name: "Redeemer",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Astral Burst",
		internalSlug: "Mod_AstralBurst_C",
		imageUrl: "items/mods/astralburst.png",
		id: "mqta6g",
		dlc: "BASE",
		description: [
			"Fires a short range burst of 7 star fragments which deal 70 -> 210 damage each. Fragments bounce off walls up to 3 times, dealing 10% additional damage per bounce. Weakspot hits deal reduced damage.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Astral_Burst`,
		location: { world: "Yaesha", dungeon: ["Cathedral of Omens"] },
	},
	{
		category: "MOD",
		name: "Awakening",
		internalSlug: undefined,
		imageUrl: "items/mods/awakening.png",
		dlc: "DLC1",
		id: "wszjeu",
		description: [
			"Incoming damage grants Awakening which increases Melee Damage by 5% per stack for 10s. Max 10 stacks.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Wrathbringer`,
		linkedItems: {
			weapon: {
				name: "Wrathbringer",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Banish",
		internalSlug: "Mod_Banish_C",
		imageUrl: "items/mods/banish.png",
		id: "k7z7r2",
		dlc: "BASE",
		description: [
			"Banishes enemies or allies into another dimension for 4s. When Banish ends, enemies take 1.25x -> 1.5x the damage from the Repulsor's primary fire for 8s.",
			"Powerful enemies won't be banished, but will take increased damage from any Repulsor while debuffed.",
		],
		communityTags: [`Multiplicative Debuffs`],
		wikiUrl: `https://remnant2.wiki.gg/Repulsor`,
		linkedItems: {
			weapon: {
				name: "Repulsor",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Beta Ray",
		imageUrl: "items/mods/betaray.png",
		internalSlug: "Mod_BetaRay_C",
		id: "5ke493",
		dlc: "BASE",
		description: [
			"Brands a target for 30s. Branded enemies that die leave a Brand at their location.",
			"Reloading or Swapping detonates Brands dealing 225 -> 675 damage. Additional Brands (3 Max) on the same target deal 50% damage. Slain enemies return 5% - 15% of Ammo and Mod Power to the weapon.",
		],
		communityTags: [`Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Alpha_/_Omega`,
		linkedItems: {
			weapon: {
				name: "Alpha-Omega",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Beyond the Veil",
		internalSlug: undefined,
		imageUrl: "items/mods/beyondtheveil.png",
		id: "puif17",
		dlc: "BASE",
		description: [
			"Neutral Evade turns to mist, granting Nightshade 5% base damage as Lifesteal for 5s. Perfect Dodge doubles duration.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Nightshade`,
		linkedItems: {
			weapon: {
				name: "Nightshade",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Big Bang",
		internalSlug: "Mod_BigBang_C",
		imageUrl: "items/mods/bigbang.png",
		id: "s7ozpe",
		dlc: "BASE",
		description: [
			"Funnels all current Mod Charges into the next shot. Projectile deals 65 -> 195 direct damage and 65 -> 195 Explosive damage per Charge consumed in a 5m radius. Explosion applies 75 -> 225 BURNING damage over 10s per Charge consumed.",
			"Additional Charges consumed increase all damage by 5%. Max 5 Charges.",
		],
		communityTags: [`Explosive Damage`, `AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Star_Shot`,
		linkedItems: {
			weapon: {
				name: "Star Shot",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Blaze",
		internalSlug: undefined,
		imageUrl: "items/mods/blaze.png",
		id: "gdY2uZ",
		dlc: "BASE",
		description: [
			"Charge Melee Attacks apply BURNING which deals 200 - 600 FIRE damage over 5s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Smolder`,
		linkedItems: {
			weapon: {
				name: "Smolder",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Blood Draw",
		internalSlug: "Mod_BloodDraw_C",
		imageUrl: "items/mods/blooddraw.png",
		id: "ss7zwt",
		dlc: "BASE",
		description: [
			"Shoots out razor-sharp Chain Shards which impale up to 5 targets within 15m, dealing 10 damage. On hit, chains are pulled towards the caster, dealing 250 damage split equally among enemies and applying 275 BLEEDING damage over 15s.",
		],
		communityTags: [`Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Blood_Draw`,
		location: { world: "Losomn", dungeon: ["Gilded Chambers"] },
	},
	{
		category: "MOD",
		name: "Bloodline",
		internalSlug: "Mod_Bloodline_C",
		imageUrl: "items/mods/bloodline.png",
		id: "yxxajn",
		dlc: "BASE",
		description: [
			"Fires a devastating blast which penetrates through all enemies in its path. Deals 150 -> 450 damage with a 25% Critical damage bonus, and 3x additional stagger.",
			"Bloodline damage increases by 50% for each enemy penetrated.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Merciless`,
		linkedItems: {
			weapon: {
				name: "Merciless",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Bloodshot",
		internalSlug: "Mod_Bloodshot_C",
		imageUrl: "items/mods/bloodshot.png",
		id: "tw8tbh",
		dlc: "BASE",
		description: [
			"While active, grants unlimited Reserve Ammo, increases Reload Speed by 50%, and shots appy 250 -> 750 BLEEDING damage over 10s. If 3 shot hit a Weakspot reloading, Merciless will automatically perform a quick Reload. Lasts 13s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Merciless`,
		linkedItems: {
			weapon: {
				name: "Corrupted Merciless",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Bloodthirst",
		internalSlug: undefined,
		imageUrl: "items/mods/bloodthirst.png",
		id: "1rw0kk",
		dlc: "BASE",
		description: [
			"Attacks from behind enemies always count as Weakspots. Charge Attacks deal 200 -> 600 BLEEDING damage over 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Assassin's_Dagger`,
		linkedItems: {
			weapon: {
				name: `Assassin's Dagger`,
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Bore",
		internalSlug: "Mod_Bore_C",
		imageUrl: "items/mods/bore.png",
		id: "iq4wjy",
		dlc: "BASE",
		searchableTags: ["Critical Chance", "Weakspot Damage"],
		description: [
			"Fires a drill projectile which bores into enemies on contact, dealing 80 damage. After fully burrowing into an enemy, creates a Weakspot which grants 65% of normal Weakspot Damage on hit. If attached to an existing Weakspot, Ranged Crit Chance is increased by 15% when attacking the drill. Lasts 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Bore`,
		location: { world: `N'Erud`, dungeon: [`The Putrid Domain`] },
	},
	{
		category: "MOD",
		name: "Chain of Command",
		internalSlug: undefined,
		dlc: "DLC1",
		imageUrl: "items/mods/chainofcommand.png",
		id: "lxt1j0",
		description: [
			"Harpoon Mode: Fire Harpoon to mark target causing primary fire to become Homing Rounds dealing 15% reduced damage. Enemies hit by Homing Rounds build Influence. Recasting exits Mode.",
			"When Influence fills, Monarch automatically reloads, creates a shockwave, and gains 20% Increased Damage and Infinite Reserves. Lasts 20s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Monarch`,
		linkedItems: {
			weapon: {
				name: "Monarch",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Chaos Driver",
		internalSlug: "Mod_ChaosDriver_C",
		imageUrl: "items/mods/chaosdriver.png",
		id: "dxnd9s",
		dlc: "BASE",
		description: [
			"Drives electrified rods which tether to other rods within 10m. Tethers last 10s and deal 20 -> 60 SHOCK damage per second.",
			"Targets take 50 -> 150 SHOCK damage per second, and 25% damage per extra rod. Increases 1.5x if linked to another rod. Rods in the same target will not link.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Enigma`,
		linkedItems: {
			weapon: {
				name: "Enigma",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Concussive Shot",
		internalSlug: "Mod_ConcussiveShot_C",
		imageUrl: "items/mods/concussiveshot.png",
		id: "an140h",
		dlc: "BASE",
		description: [
			"Fires a focused blast of air through all targets within 8m, dealing 155 - 465 damage and 4x impact.",
		],
		communityTags: [`AOE/Aura`, "Explosive Damage"],
		wikiUrl: `https://remnant2.wiki.gg/Concussive_Shot`,
		location: { world: "Ward 13", dungeon: "Vendor" },
	},
	{
		category: "MOD",
		name: "Corrosive Rounds",
		internalSlug: "Mod_CorrosiveRounds_C",
		imageUrl: "items/mods/corrosiverounds.png",
		id: "hr3g7o",
		dlc: "BASE",
		searchableTags: ["Critical Chance"],
		description: [
			"Imbues ammunition with ACID and increases Range Critical Chance by 15% for 20s. Shots apply CORRODED dealing 150 - 450 damage over 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Corrosive_Rounds`,
		location: { world: "Losomn", dungeon: ["Shattered Gallery"] },
	},
	{
		category: "MOD",
		name: "Creeping Mist",
		internalSlug: "Mod_CreepingMist_C",
		imageUrl: "items/mods/creepingmist.png",
		dlc: "DLC1",
		id: "a3ctlf",
		description: [
			"Shoots out a canister that shatters to reveal an expanding mist, starting at 7.5m and growing to 15m over 5s. Lasts 20s.",
			"Enemies within the mist receive 25% additional Status Effect Damage and are 5% more likely to be struck with a critical hit from any source. The mists debuff persists for 10s after targets exit the mist.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Creeping_Mist`,
		location: { world: "Losomn", dungeon: ["Sunken Haunt"] },
	},
	{
		category: "MOD",
		name: "Cube Room",
		internalSlug: "Mod_CubeRoom_C",
		imageUrl: "items/mods/cuberoom.png",
		id: "2nMTRG",
		dlc: "BASE",
		description: [
			"Conjures a 5m anomalous Cube Room which grants allies an increase of 75% Heat Decay Rate, reduces Heat Generation per round when firing weapons that Overheat by 20% and automatically generates missing reserve ammo per second. Lasts 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Cube_Gun`,
		linkedItems: {
			weapon: {
				name: "Corrupted Cube Gun",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Cube Shield",
		internalSlug: "Mod_CubeShield_C", // todo test this on loadout import
		imageUrl: "items/mods/cubeshield.png",
		id: "1bfanq",
		dlc: "BASE",
		description: [
			"Generates a Cube Shield which absorbs up to 500 -> 1500 damage from incoming enemy projectiles. Lasts 15s.",
			"Reactivating fires the Cube Shield which damages enemies as it travels. Damage is increased if the Cube Shield has absorbed enemy projectile damage before firing.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Cube_Gun`,
		linkedItems: {
			weapon: {
				name: "Cube Gun",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Cyclone",
		internalSlug: undefined,
		imageUrl: "items/mods/cyclone.png",
		id: "1bfadq",
		dlc: "DLC2",
		description: [
			"Maintaining an overhead Charged Melee forms a 5m sand cyclone, dealing 35 damage per second and pulls enemies towards the wielder. Each Cyclone hit applies EXPOSED for 3s. Max 15s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Mirage`,
		linkedItems: {
			weapon: {
				name: "Mirage",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Deadpoint",
		internalSlug: "Mod_Mortar_C",
		imageUrl: "items/mods/corrupted_deadpoint.png",
		id: "ysz4fw",
		dlc: "BASE",
		description: [
			"Fires a cluster bomb which detonates on impact across 2.5m. Each cluster deals up to 50 -> 150 damage per explosion within 3.5m.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Meridian`,
		linkedItems: {
			weapon: {
				name: "Corrupted Meridian",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Deadwood",
		internalSlug: "Mod_Deadwood_C",
		imageUrl: "items/mods/deadwood.png",
		communityTags: ["Explosive Damage"],
		id: "k7z7rz",
		dlc: "DLC2",
		description: [
			"Disables automatic BLOWBACK, while increasing Thorn's Reload Speed by 25%, and Fire Rate by 15%.",
			"Reactivating the Mod will trigger an EMPOWERED BLOWBACK, which detonates all needles at once for 100% additional damage. Lasts 20s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Thorn`,
		linkedItems: {
			weapon: {
				name: "Thorn",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Death Brand",
		internalSlug: "Mod_DeathBrand_C",
		imageUrl: "items/mods/corrupted_deathbrand.png",
		id: "d3q99g",
		dlc: "BASE",
		description: [
			"Applies Death Brand to all non-allies within 25m, increasing incoming damage by 10%. Enemies killed while bearing the Death Brand leave behind an Echo which lasts 10s before returning to the Nightmare Realm.",
			"Echoes are drawn to any survivor who walks within 3m, increasing Weakspot Damage dealt by 10% for 10s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Rune_Pistol`,
		linkedItems: {
			weapon: {
				name: "Corrupted Rune Pistol",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Death Sentence",
		internalSlug: undefined,
		imageUrl: "items/mods/feraljudgement.png",
		id: "wilhsl",
		dlc: "BASE",
		description: [
			"Dealing Melee Damage 6 times over 10s empowers Feral Judgement. When empowered, Neutral Backdash Charge Attacks apply Death Sentence.",
			"After 1.5s of being sentenced, the enemy will suffer 10 Phantom Strikes, dealing 25 -> 75 Damage each. Deals 25% additional damage to BLEEDING enemies.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Feral_Judgement`,
		linkedItems: {
			weapon: {
				name: "Feral Judgement",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Defrag",
		internalSlug: "Mod_Defrag_C",
		imageUrl: "items/mods/defrag.png",
		id: "3hqdob",
		dlc: "BASE",
		description: [
			"Infects weapon with Malware for 30s, causing shots to apply FRAGMENTED for 5s. When a FRAGMENTED enemy dies, they create a Glitch that lasts 15s.",
			"Picking up a Glitch increases All damage by 20% for 15s.",
			"Destroying a Glitch causes it to destabilize, creating a 5m Virus Pool which deals 25 damage per second and applies the FRAGMENTED debuff to enemies inside. Virus Pools last 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Defrag`,
		location: { world: "Root Earth", dungeon: ["Ashen Wasteland"] },
	},
	{
		category: "MOD",
		name: "Dreadwalker",
		internalSlug: "Mod_Dreadwalker_C",
		imageUrl: "items/mods/dreadwalker.png",
		id: "ybej3w",
		dlc: "BASE",
		description: [
			"Enter the Nightmare Realm. Nightfall gains infinite Ammo, a 25% Fire Rate increase, 10% Lifesteal, and becomes fully automatic. The wielder becomes significantly harder to hit while moving.",
			"Disables other weapons and Skills for the duration, or until Dreadwalker is deactivated. Lasts 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Nightfall`,
		linkedItems: {
			weapon: {
				name: "Nightfall",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Dreamwave",
		internalSlug: undefined,
		imageUrl: "items/mods/dreamwave.png",
		id: "dwmc70",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"After dealing 250 -> 750 damage, Charge Attack will release a Dreamwave, floating outwards 20m and returning to caster.",
			"Dreamwave applies SLOW to all enemies for 10s and grants a Stack of REVERIE for each enemy affected. Each Stack grants 2% to All Damage and 2% Movement Speed, which lasts 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Dreamcatcher`,
		linkedItems: {
			weapon: {
				name: "Dreamcatcher",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Dying Breath",
		internalSlug: undefined,
		imageUrl: "items/mods/dyingbreath.png",
		id: "22nk8x",
		dlc: "BASE",
		description: [
			"When Infused, Neutral Backdash Charge Attack explodes in a 3m AOE and leaves an ACID Cloud which lasts for 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Gas_Giant`,
		linkedItems: {
			weapon: {
				name: "Gas Giant",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Energy Wall",
		imageUrl: "items/mods/energywall.png",
		internalSlug: "Mod_EnergyWall_C", // todo test this on loadout import
		id: "8uluwc",
		dlc: "BASE",
		description: [
			"Deploys an energy barrier on impact with ground. Allies can shoot through Energy Wall but enemy projectiles are absorbed (up to 500 damage received). Max 1 Wall at a time. Lasts 30s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Energy_Wall`,
		location: { world: `N'Erud`, dungeon: ["Timeless Horizon"] },
	},
	{
		category: "MOD",
		name: "Energy Wave",
		internalSlug: undefined,
		imageUrl: "items/mods/energywave.png",
		id: "7ztoh8",
		dlc: "BASE",
		description: [
			"Charge Attacks use Stamina to release an energy wave projectile, allowing the wielder to strike enemies from much farther away.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Hero's_Sword`,
		linkedItems: {
			weapon: {
				name: "Hero's Sword",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Eulogy",
		internalSlug: "Mod_Eulogy_C",
		imageUrl: "items/mods/eulogy.png",
		id: "3xf7i2",
		dlc: "BASE",
		description: [
			"Recalls bolts which deal 30 -> 90 damage when pulled from a target and when striking targets on their return. Recalled bolts grant 2% of Max Health.",
			"Recalled bolts can also overfill Sorrow up to +5. Any additional bolts will be returned to reserves.",
		],
		communityTags: [`Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Sorrow`,
		linkedItems: {
			weapon: {
				name: "Sorrow",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Explosive Shot",
		internalSlug: "Mod_ExplosiveShot_C",
		imageUrl: "items/mods/explosiveshot.png",
		id: "hpfdzp",
		dlc: "BASE",
		description: [
			"Fires an explosive round that deals up to 175 -> 525 FIRE damage within 9m and applies BURNING, dealing 250 -> 750 FIRE damage over 10s.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Hellfire`,
		linkedItems: {
			weapon: {
				name: "Hellfire",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Familiar",
		internalSlug: "Mod_Familiar_C",
		imageUrl: "items/mods/familiar.png",
		id: "sccdmt",
		dlc: "BASE",
		description: [
			"Summons Fearie Familiar to aid in combat. The Familiar selects a random enemy within 10m and slashes through them for 25 damage each attack. Familiar will select a new target when the previous one dies. Lasts 20s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Familiar`,
		location: { world: "Losomn", dungeon: [`Briella's Reverie`] },
	},
	{
		category: "MOD",
		name: "Fargazer",
		internalSlug: "Mod_Fargazer_C",
		imageUrl: "items/mods/fargazer.png",
		id: "yedqs6",
		dlc: "BASE",
		description: [
			"Calls forth an eye of Legion to gaze at the player's AIM target. For every 0.25s Fargazer focuses on a target within 25m, a stack of MADNESS Status is applied for 5s. Each stack deals 3 Damage per second. Max 10 stacks. Lasts 30s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Fargazer`,
		location: { world: "Yaesha", dungeon: ["The Twisted Chantry"] },
	},
	{
		category: "MOD",
		name: "Fathomless Deep",
		internalSlug: undefined,
		imageUrl: "items/mods/fathomlessdeep.png",
		dlc: "DLC1",
		id: "o1zwol",
		description: [
			"Charged Melee Hits grant Fathomless Deep which increases Melee Stagger by 10% per stack for 5s. Max 3 stacks.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Abyssal_Hook`,
		linkedItems: {
			weapon: {
				name: "Abyssal Hook",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Faultline",
		internalSlug: undefined,
		imageUrl: "items/mods/faultline.png",
		id: "5mlisj",
		dlc: "BASE",
		description: [
			"Melee Charge attacks fire off ground-based shockwaves which deal 115 -> 345 damage.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Stonebreaker`,
		linkedItems: {
			weapon: {
				name: "Stonebreaker",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Firestorm",
		internalSlug: "Mod_Firestorm_C",
		imageUrl: "items/mods/firestorm.png",
		id: "snlx8w",
		dlc: "BASE",
		description: [
			"Creates a whirling cyclone that sucks in nearby targets and applies BURNING for 10s. The center of the cyclone deals 75 FIRE damage per second. Lasts 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Firestorm`,
		location: { world: "Losomn", dungeon: ["Gilded Chambers"] },
	},
	{
		category: "MOD",
		name: "Fission Strike",
		internalSlug: undefined,
		imageUrl: "items/mods/fissionstrike.png",
		id: "d490vt",
		dlc: "BASE",
		description: [
			"On Neutral Evade Attacks Atom Splitter achieves Nuclear Fission, releasing a wave of charged particles which deals 150 -> 450 damage to targets within 20m.",
			"Charged Neutral Evade Attacks increase range by 3x and damage by 25%.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Atom_Splitter`,
		linkedItems: {
			weapon: {
				name: "Atom Splitter",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Flying Bomb Trap",
		internalSlug: "Mod_FlyingBombTrap_C",
		imageUrl: "items/mods/flyingbombtrap.png",
		id: "2GWpcL",
		dlc: "DLC2",
		communityTags: ["AOE/Aura"],
		description: [
			"Fires a projectile that deploys up to 5 traps after contacting the ground.",
			"Each trap can launch a flask of deadly liquid up to 15m when an enemy travels above and will rearm once every 1.5s. Flask detonations release a 7m shockwave which deals 50 Damage and splashes targets within 4m of the center for 100 ACID Damage, applying CORRODED for 100 damage over 10s.",
			"Lasts 20s or 10 total flask launches.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Flying_Bomb_Trap`,
		location: { world: "Yaesha", dungeon: ["Proving Grounds"] },
	},
	{
		category: "MOD",
		name: "Fracture",
		internalSlug: undefined,
		imageUrl: "items/mods/fracture.png",
		id: "x8vmra",
		dlc: "BASE",
		description: [
			"Charge Attacks taint the blood of targets, causing all attacks from Godsplitter to register as Weakspot hits for 2s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Godsplitter`,
		linkedItems: {
			weapon: {
				name: "Godsplitter",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Fusion Cannon",
		internalSlug: "Mod_FusionCannon_C",
		imageUrl: "items/mods/fusioncannon.png",
		id: "D8obdv",
		dlc: "BASE",
		description: [
			"Funnels all current Mod Charges into a powerful penetrating rail dealing 100 -> 300 base damage which is increased per Charge consumed. Max 5 Charges.",
			"Consuming a Charge refills 20% of the Primary Magazine.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Savior`,
		linkedItems: {
			weapon: {
				name: "Corrupted Savior",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Gravity Core",
		internalSlug: "Mod_GravityCore_C",
		imageUrl: "items/mods/gravitycore.png",
		id: "kep3kb",
		dlc: "BASE",
		description: [
			"Fires a highly compressed sphere which creates a large Gravity Well on contact, pulling in enemies for 4s.",
			"When the Gravity Well expires, it deals 750 -> 2250 damage within 10m. Adds 500 -> 1500 damage per enemy and divides total damage among all enemies.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Starkiller`,
		linkedItems: {
			weapon: {
				name: "Starkiller",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Guardian's Call",
		internalSlug: "Mod_GuardiansCall_C",
		imageUrl: "items/mods/guardianscall.png",
		id: "anhbhf",
		dlc: "BASE",
		description: [
			"When Mod Power is full, Primary Fire becomes Empowered and highlights enemies it strikes.",
			"Activating the Mod calls down a Guardian Sword on enemies struck by an Empowered Energy Disc. Swords deal 150 -> 450 damage and 3x stagger within 3m.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Twisted_Arbalest`,
		linkedItems: {
			weapon: {
				name: "Twisted Arbalest",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: `Guardian's Fury`,
		internalSlug: "Mod_GuardiansVigil_C",
		imageUrl: "items/mods/guardiansfury.png",
		id: "WzXG4x",
		dlc: "BASE",
		description: [
			"Imbues the Arbalest with the power of Yaesha's corrupted guardian, increases Fire Rate by 20%, reduces Recoil and Sway by 25% and each disc impact with an enemy explodes over 3.5m for 69 -> 207 damage. Lasts 15s.",
		],
		communityTags: [`AOE/Aura`, "Explosive Damage"],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Arbalest`,
		linkedItems: {
			weapon: {
				name: "Corrupted Arbalest",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Harvester Bots",
		internalSlug: "Mod_HarvesterBots_C",
		imageUrl: "items/mods/harvesterbots.png",
		id: "vk6XTW",
		dlc: "DLC3",
		searchableTags: ["Lifesteal"],
		description: [
			"Releases 5 Harvester Bots for 30s that seek an enemy to siphon, granting allies within 30m a stack of DRAIN.",
			"DRAIN: Gain 1% of base damage dealt as Lifesteal per stack. Max 5 stacks.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Harvester_Bots`,
		location: { world: `N'Erud`, dungeon: [`Detritus Foundry`] },
	},
	{
		category: "MOD",
		name: "Heatwave",
		internalSlug: "Mod_Heatwave_C",
		imageUrl: "items/mods/heatwave.png",
		dlc: "DLC2",
		id: "vc3s1n",
		communityTags: ["AOE/Aura"],
		description: [
			"Activates a 15m aura of sweltering heat, causing enemies inside the aura for 3s to begin BURNING for 250 FIRE Damage over 5s. Lasts 30s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Heatwave`,
		location: { world: "Yaesha", dungeon: ["Deserted Atelier"] },
	},
	{
		category: "MOD",
		name: "Healing Shot",
		internalSlug: "Mod_HealingShot_C",
		imageUrl: "items/mods/healingshot.png",
		id: "vc361n",
		dlc: "BASE",
		description: [
			"Launches a payload that explodes on contact with allies, healing 35% of their max health. When no ally is struck, payload lays dormant until an ally gets close. Dormant payload lasts 30s, slowly losing healing potency over time.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Healing_Shot`,
		location: { world: "Ward 13", dungeon: "Vendor" },
	},
	{
		category: "MOD",
		name: "Heat Sink",
		internalSlug: "Mod_HeatSink_C",
		imageUrl: "items/mods/heatsink.png",
		id: "s49h0c",
		dlc: "BASE",
		description: [
			"Forces open Plasma Cutter's heat vents dispersing all heat. While active, Plasma Cutter generates 50% less heat, and ramping damage cap is increased to 3x Damage. Overheats automatically when deactivated. Lasts 20s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Plasma_Cutter`,
		linkedItems: {
			weapon: {
				name: "Plasma Cutter",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Helix",
		internalSlug: "Mod_Helix_C",
		imageUrl: "items/mods/helix.png",
		communityTags: ["Explosive Damage"],
		id: "1mtqkk",
		dlc: "BASE",
		description: [
			"Shoots a helix of missiles, dealing 120 - 360 damage. On contact, divides into 6 smaller rockets which seek additional targets, dealing 30 - 90 damage on contact.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Helix`,
		location: { world: `N'Erud`, dungeon: [`Astropath's Respite`] },
	},
	{
		category: "MOD",
		name: "Horizon Strike",
		internalSlug: undefined,
		imageUrl: "items/mods/horizonstrike.png",
		id: "2fjw9q",
		dlc: "BASE",
		description: [
			"Charge Attacks release a wide horizontal projectile that penetrates through multiple enemies dealing 100.8 -> 302.4 damage.",
		],
		wikiUrl: `https://remnant2.wiki.gg/World's_Edge`,
		linkedItems: {
			weapon: {
				name: "World's Edge",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Hot Shot",
		internalSlug: "Mod_Hotshot_C",
		imageUrl: "items/mods/hotshot.png",
		id: "9vzb9r",
		dlc: "BASE",
		description: [
			"Imbues ammunition with FIRE and increases Ranged damage by 15% for 20s. Shots also apply BURNING, dealing 200 - 600 FIRE Damage over 10 seconds.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Hot_Shot`,
		location: { world: "Ward 13", dungeon: "Vendor" },
	},
	{
		category: "MOD",
		name: "Indomitable Strike",
		internalSlug: "Mod_IndomitableStrike_C",
		imageUrl: "items/mods/indomitablestrike.png",
		id: "778jv8",
		dlc: "DLC3",
		searchableTags: [
			"Damage Reduction",
			"Grey Health",
			"Stagger",
			"Charged Melee Damage",
		],
		description: [
			"Charged Attacks become INDOMITABLE.",
			"INDOMITABLE: Grants 15% Damage Reduction, converts all damage taken to Grey Health, and prevents wielder from being staggered. Charging increases maximum damage by up to 350%.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Black_Greatsword`,
		location: { world: "Any", dungeon: "Linked Item" },
		linkedItems: {
			weapon: {
				name: "Black Greatsword",
			},
		},
	},
	{
		category: "MOD",
		name: "Javelin",
		internalSlug: undefined,
		imageUrl: "items/mods/athibar.png",
		id: "29kz6t",
		dlc: "BASE",
		description: [
			"Charge attacks use 25 stamina to throw the spear, which returns to the wielder's hand.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Huntress_Spear`,
		linkedItems: {
			weapon: {
				name: "Huntress Spear",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Kilonova",
		internalSlug: "Mod_Kilonova_C",
		imageUrl: "items/mods/kilonova.png",
		id: "4hu8s0",
		dlc: "DLC3",
		description: [
			"Charged Melee empowers the Gauntlets for 15s, increasing Standard Melee Damage by 25% and causing Charged Melee Attacks to release 8m penetrating blasts. Each blast reduces the duration.",
			"While empowered, performing a Charged Melee Attack during a Neutral Backdash gathers power for up to 5s, then fires a DARKNESS BEAM that deals 100 -> 300 Melee Damage every 0.5s. The beam duration equals its time charged, then ends Kilonova after firing.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Dark_Matter_Gauntlets`,
		linkedItems: {
			weapon: {
				name: "Dark Matter Gauntlets",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Knight Guard",
		internalSlug: "Mod_KnightGuard_C",
		imageUrl: "items/mods/knightguard.png",
		dlc: "DLC1",
		id: "e4n1w4",
		description: [
			"Summons Knight Guard to aid in combat. The Guards hurl penetrating slashes at a random enemy dealing 65 damage. The Guards will melee enemies if close enough for 65 damage. Lasts 30s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Knight_Guard`,
		location: { world: "Losomn", dungeon: ["Walk of Rememberance"] },
	},
	{
		category: "MOD",
		name: "Krell Edge",
		internalSlug: undefined,
		imageUrl: "items/mods/krelledge.png",
		id: "38rncx",
		dlc: "BASE",
		description: [
			"Charge to throw the Krell Axe which applies OVERLOADED on hit, dealing 50 - 150 SHOCK damage every 5s for 10s. Shortly after throwing, another will appear in the wielders hand.",
			"Costs 25 Stamina.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Krell_Axe`,
		linkedItems: {
			weapon: {
				name: "Krell Axe",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Lifeline",
		internalSlug: undefined,
		imageUrl: "items/mods/lifeline.png",
		id: "4zqfhu",
		dlc: "BASE",
		description: [
			"After dealing 250 - 750 damage, the next Charge Attack causes the Spirit of the Red Doe to stampede forward, dealing 160  - 480 damage to enemies and regenerating 10% Health to allies.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Red_Doe_Staff`,
		linkedItems: {
			weapon: {
				name: "Red Doe Staff",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Loathe the Weak",
		internalSlug: "Mod_LoathTheWeak_C",
		imageUrl: "items/mods/loaththeweak.png",
		communityTags: ["Explosive Damage"],
		dlc: "DLC1",
		id: "aobtw7",
		description: [
			"Rapidly fires volatile needles that explode after 1.5s dealing 30 -> 90 explosive damage. Deals additional damage to targets with lower health. Lasts 15s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Anguish`,
		linkedItems: {
			weapon: {
				name: "Anguish",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Mega Drive",
		internalSlug: "Mod_MegaDrive_C",
		imageUrl: "items/mods/megadrive.png",
		id: "5s9x12",
		dlc: "DLC3",
		description: [
			"Activate while a valid target is in-sights, marking it with quantum Entanglement. All Genesis projectiles seek to the Entanglement target after initial impact, dealing 35% of Primary Fire damage.",
			"While active, activating can switch Entanglement target. Lasts 15s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Genesis`,
		linkedItems: {
			weapon: {
				name: "Genesis",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Micronova",
		internalSlug: "Mod_Micronova_C",
		imageUrl: "items/mods/corrupted_micronova.png",
		id: "8izb2g",
		dlc: "BASE",
		description: [
			"Fires 5 shattered star orbs. On contact, they each explode for 75 -> 225 FIRE damage, and apply 200 -> 600 BURNING damage over 10s to all targets within 2.75m.",
			"Striking the Micronova with Corrupted Aphelion's primary fire causes a chain reaction of larger and more powerful detonations which also apply the initial BURNING amount.",
			"Generates 1 Primary Ammo on Fire.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Aphelion`,
		linkedItems: {
			weapon: {
				name: "Corrupted Aphelion",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Moonlight Barrage",
		internalSlug: "Mod_MoonlightBarrage_C",
		imageUrl: "items/mods/moonlightbarrage.png",
		id: "wfak3h",
		dlc: "BASE",
		description: [
			"Empowers the Bow for 15s. Arrows apply Moonlight to enemies for 3s. Enemies struck by Crescent Moon while Moonlit release a Moon Essence which returns 1 arrow, heals 5% of Max Health, and grants 15% Fire and Reload Speed for 5s.",
			"Basic Shots automatically become Charged Shots. Perfect Shots fire two arrows.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Crescent_Moon`,
		linkedItems: {
			weapon: {
				name: "Crescent Moon",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Nano Phase",
		internalSlug: "Mod_NanoPhase_C",
		imageUrl: "items/mods/nanophase.png",
		id: "WARGv3",
		dlc: "BASE",
		description: [
			"Detonate active Nano-bots, each dealing 100 -> 300 ACID damage plus an additional 25 -> 75 ACID damage per Nano-bot detonated to enemies within 2m and applies CORRODED, dealing 50 -> 150 damage over 10s per Nano-bot detonated.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Nebula`,
		linkedItems: {
			weapon: {
				name: "Corrupted Nebula",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Nano Swarm",
		internalSlug: "Mod_NanoSwarm_C",
		imageUrl: "items/mods/nanoswarm.png",
		id: "8ko9zz",
		dlc: "BASE",
		description: [
			"Unleash a swarm of Nanomachines that seek after enemies within 20 meters and repeatedly attack dealing 3 -> 9 ACID damage per hit and applying CORRODED dealing 50 -> 150 ACID damage over 5s. Lasts 10 seconds.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Nebula`,
		linkedItems: {
			weapon: {
				name: "Nebula",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Ouroboros",
		internalSlug: "Mod_Ouroboros_C",
		imageUrl: "items/mods/ouroboros.png",
		id: "czgb8w",
		dlc: "BASE",
		description: [
			"Conjures 3 sword fragments that encircle the wielder for 30s. Performing a Melee Attack lets loose a Fragment which taints the blood of targets, causing all attacks from Deceit to register as Weakspot hits for a short period of time.",
			"Charge Melee Attacks fire all remaining Fragments at once but they taint blood for a shorter duration for each thrown.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Deceit`,
		linkedItems: {
			weapon: {
				name: "Deceit",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Overflow",
		internalSlug: "Mod_Overflow_C",
		imageUrl: "items/mods/overflow.png",
		id: "tya9an",
		dlc: "BASE",
		description: [
			"Imbues ammunition with SHOCK and increases Fire Rate by 15% and Reload Speed by 15% for 20 seconds. Shots also apply OVERLOADED, dealing 35 SHOCK Damage every 5s for 15s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Overflow`,
		location: {
			world: `N'Erud`,
			dungeon: ["Ascension Spire"],
		},
	},
	{
		category: "MOD",
		name: "Phantom Blade",
		internalSlug: "Mod_PhantomBlade_C",
		imageUrl: "items/mods/phantomblade.png",
		id: "t1ugnb",
		dlc: "DLC3",
		description: [
			"Charged Melee Attacks release a Phantom that trails behind the host weapon, dealing 35% of the Harvester Scythe’s damage. Each hit of the Phantom Blade increases Melee Attack Speed and Melee Charge Speed by 6% for 5s. Max 5 stacks.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Harvester_Scythe`,
		linkedItems: {
			weapon: {
				name: "Harvester Scythe",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Power Stone",
		internalSlug: undefined,
		imageUrl: "items/mods/powerstone.png",
		id: "2JimAt",
		dlc: "DLC2",
		searchableTags: ["Status Effect"],
		description: [
			"Charged Melee empowers the staff for 10s, causing Melee Attacks to launch an energy bolt that seeks enemies within 30m, dealing 50 -> 150 SHOCK Damage and grants 1 stack of POWER STONE. Max 10 stacks.",
			"While empowered, Charged Melee calls a 6m shockwave dealing 75 -> 225 SHOCK Damage and ends POWER STONE. For each POWER STONE stack built when ended, release 1 energy bolt that seeks a random enemy and applies OVERLOADED, dealing 50 -> 150 SHOCK damage every 5s for 10s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Crystal_Staff`,
		linkedItems: {
			weapon: {
				name: "Crystal Staff",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Prismatic Driver",
		internalSlug: "Mod_PrismaticDriver_C", // todo test this on loadout import
		imageUrl: "items/mods/prismaticdriver.png",
		id: "vtcg3o",
		dlc: "BASE",
		description: [
			"Fires a superheated beam which deals 125 Mod Damage per second. Sustaining the beam on a target causes an explosion which deals 300 Mod damage in a 3m AOE.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Prismatic_Driver`,
		location: { world: `N'Erud`, dungeon: [`Spectrum Nexus`] },
	},
	{
		category: "MOD",
		name: "Reaver",
		internalSlug: undefined,
		imageUrl: "items/mods/reaver.png",
		dlc: "DLC1",
		id: "geb8ho",
		description: [
			"Increases Melee Damage by 10% if the target is suffering from a Negative Status Effect.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Ritualist_Scythe`,
		linkedItems: {
			weapon: {
				name: "Ritualist Scythe",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Recoil Dampener",
		internalSlug: "Mod_RecoilDampener_C",
		imageUrl: "items/mods/recoildampener.png",
		id: "Rix6Sr",
		dlc: "DLC3",
		searchableTags: [
			"Charged Shot",
			"Recoil",
			"Critical Chance",
			"Critical Damage",
		],
		description: [
			"Activation stabilizes Monorail, increasing Charge Speed by 25% and reducing Recoil and Sway by 75%. Lasts 20s.",
			"Additionally, for each 5% of the Charge Meter filled upon firing, gain 1% Critical Chance and Critical Damage. Max 20%.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Monorail`,
		linkedItems: {
			weapon: {
				name: "Monorail",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Ring of Spears",
		internalSlug: "Mod_RingOfSpears_C",
		imageUrl: "items/mods/ringofspears.png",
		dlc: "DLC1",
		id: "4rdl8r",
		description: [
			"Calls forth 7 Phantom Spears to encircle the wielder for 25s. Deals up to 35 damage per second to nearby enemies, based on the number of spears remaining in the ring.",
			"While active, spears can be thrown by reactivating the Mod. Spears deal 100 and will remain at their impact point for the duration of the Mod.",
			"Holding the Mod Button recalls embedded Spears, dealing 50 damage to enemies along their path. When all recalled spears return, they cause an Explosion dealing up to 350 damage based on the number of spears recalled.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Ring_of_Spears`,
		location: { world: "Losomn", dungeon: ["Glistering Cloister"] },
	},
	{
		category: "MOD",
		name: "Rootlash",
		internalSlug: "Mod_Rootlash_C",
		imageUrl: "items/mods/rootlash.png",
		id: "nfw6ja",
		dlc: "BASE",
		description: [
			"Launches a projectile which summons a Root Tentacle. Tentacles deal 30 - 90 damage and steal 1.5% of the hero's Max Health per hit. Lasts 20s. (Max 2)",
		],
		wikiUrl: `https://remnant2.wiki.gg/Rootlash`,
		location: { world: "Yaesha", dungeon: [`Kaeula's Rest`] },
	},
	{
		category: "MOD",
		name: "Rotted Arrow",
		internalSlug: "Mod_RottedArrow_C",
		imageUrl: "items/mods/rottedarrow.png",
		id: "vgin4w",
		dlc: "BASE",
		description: [
			"Fires a rotten arrow that deals 110 - 330 Base Damage with a 150% bonus Weakspot modifier. Detonates for another 100 - 300 damage within 4m.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Rotted_Arrow`,
		location: { world: "Yaesha", dungeon: ["The Expanding Glade"] },
	},
	{
		category: "MOD",
		name: "Sandstorm",
		internalSlug: "Mod_Sandstorm_C",
		imageUrl: "items/mods/sandstorm.png",
		dlc: "DLC2",
		id: "ozNPv9",
		searchableTags: [],
		description: [
			"Launch a sphere of loam. Impact creates a 5m swirling storm of sand which seeks to center itself on an enemy and deals 45 -> 135 Elemental damage per second. When Sandstorm's focused target dies, it will seek a new enemy within 10m. Lasts 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Monolith`,
		linkedItems: {
			weapon: {
				name: "Monolith",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Scrap Shot",
		internalSlug: "Mod_ScrapShot_C",
		imageUrl: "items/mods/scrapshot.png",
		id: "hcv400",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Fires a canister that releases Caltrops over an area of 6m. Caltrops apply SLOW and BLEEDING to enemies, dealing 400 - 1,200 damage over 10s. Lasts 10s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Scrap_Shot`,
		location: { world: "Ward 13", dungeon: "Vendor" },
	},
	{
		category: "MOD",
		name: "Screamer",
		internalSlug: "Mod_Screamer_C",
		imageUrl: "items/mods/screamer.png",
		id: "4aeebu",
		dlc: "BASE",
		description: [
			"Fires a high-powered rocket that deals up to 200 -> 600 damage within 2.5m.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Meridian`,
		linkedItems: {
			weapon: {
				name: "Meridian",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Shatterstar",
		internalSlug: "Mod_Shatterstar_C",
		imageUrl: "items/mods/shatterstar.png",
		id: "z9e8qq",
		dlc: "BASE",
		description: [
			"Imbues ammunition with FIRE. Hits apply BURNING, which deals 250 -> 750 FIRE damage over 10 seconds. Lasts 20s.",
			"Hitting an enemy causes the shot to fragment, striking enemies within 7m for 30% of the initial damage dealt and also apply BURNING. Lasts 20s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Savior`,
		linkedItems: {
			weapon: {
				name: "Savior",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Shielding Shot",
		internalSlug: "Mod_ShieldingShot_C",
		imageUrl: "items/mods/shieldingshot.png",
		id: "DUs5M2",
		dlc: "DLC3",
		description: [
			"Launches a payload that bursts on contact with allies, applying a SHIELD for 25% of their Max Health for 15s. Max 50% SHIELD. When no ally is struck, payload lays dormant until an ally gets close. Dormant payload lasts 30s and slowly loses Shield potency over time.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Shielding_Shot`,
		location: { world: `N'Erud`, dungeon: [`Athenaeum Wek`] },
	},
	{
		category: "MOD",
		name: "Skewer 2.0",
		internalSlug: "Mod_Skewer_C",
		imageUrl: "items/mods/skewer.png",
		id: "cpvech",
		dlc: "BASE",
		description: [
			"Fires a Wretched Spear which embeds itself on contact. Spears deal 125 damage on hit, rapidly dividing inside the target until bursting, dealing 140 damage to all targets within 3m. Spears embedded in the environment remain in place for 10s.",
		],
		communityTags: ["AOE/Aura"],
		wikiUrl: `https://remnant2.wiki.gg/Skewer_2.0`,
		location: { world: "Root Earth", dungeon: ["Corrupted Harbor"] },
	},
	{
		category: "MOD",
		name: "Song of Eafir",
		internalSlug: "Mod_SongOfEafir_C",
		imageUrl: "items/mods/songofeafir.png",
		id: "edm644",
		dlc: "BASE",
		searchableTags: ["Status Effect", "Damage Reduction"],
		description: [
			"Fires a shot infused with the binding power of the Song of Eafir. Staggers most ground enemies within 10m and deals 150 -> 450 damage to Flying enemies within the same range.",
			"The song continues for 15s, afflicting targets within 15m with SLOW reducing their damage by 10%.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Song_of_Eafir`,
		location: { world: "Yaesha", dungeon: ["The Forbidden Grove"] },
	},
	{
		category: "MOD",
		name: "Soul Brand",
		internalSlug: "Mod_Soulbrand_C",
		imageUrl: "items/mods/soulbrand.png",
		id: "imk8xh",
		dlc: "BASE",
		description: [
			"Applies Soul Brand to all enemies within 25m which lasts 25s. Enemies killed while bearing the Soul Brand leave behind an Echo which lasts 15s before returning to the Nightmare Realm.",
			"Echoes are drawn to any survivor who walks within 3m, granting 25% of Max Health on contact.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Rune_Pistol`,
		linkedItems: {
			weapon: {
				name: "Rune Pistol",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Soulbinder",
		internalSlug: "Mod_Soulbinder_C",
		imageUrl: "items/mods/soulbinder.png",
		id: "5bb0mv",
		dlc: "BASE",
		description: [
			"Fires a projectile that attaches to the enemy dealing 40 damage. Enemies within 7m become bound to the primary target after impact and share 60% of damage dealt to them. Lasts 15s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Soulbinder`,
		location: { world: "Yaesha", dungeon: ["The Chimney"] },
	},
	{
		category: "MOD",
		name: "Space Crabs",
		internalSlug: "Mod_SpaceCrabs_C",
		imageUrl: "items/mods/spacecrabs.png",
		communityTags: ["Explosive Damage"],
		id: "4wd3gm",
		dlc: "BASE",
		description: [
			"Launch an alien egg that bursts on impact, releasing 5 Space Crabs. Crabs follow the caster, leaping towards enemies within 4m, and exploding, dealing 60 - 180 damage each.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Space_Crabs`,
		location: { world: `N'Erud`, dungeon: [`The Hatchery`] },
	},
	{
		category: "MOD",
		name: "Spore Shot",
		internalSlug: "Mod_SporeShot_C",
		imageUrl: "items/mods/sporeshot.png",
		id: "9fi67v",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Fires a rotten spore that releases a specialized gas cloud over 6m. Applies SLOW to enemies while dealing 15 -> 45 damage per second for 7s. Grants HASTE to allies, which lasts 5s after leaving the cloud.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Sporebloom`,
		linkedItems: {
			weapon: {
				name: "Sporebloom",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Starfall",
		internalSlug: "Mod_Starfall_C",
		imageUrl: "items/mods/starfall.png",
		id: "j9wkb3",
		dlc: "BASE",
		description: [
			"Fires a magical arrow which deals 30 -> 90 damage and opens a 7m portal that rains down star fragments. Each fragment deals 50 -> 150 damage within 2m. Last 6s.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Sagittarius`,
		linkedItems: {
			weapon: {
				name: "Sagittarius",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Stasis Beam",
		internalSlug: "Mod_StasisBeam_C",
		imageUrl: "items/mods/stasisbeam.png",
		id: "rer73h",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Fires a beam which deals 15 - 45 damage per second, and applies SLOW Debuff. After 2s of application to an enemy, SLOW becomes STASIS, freezing the target in place for 10s.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Stasis_Beam`,
		location: { world: `N'Erud`, dungeon: ["The Eon Vault"] },
	},
	{
		category: "MOD",
		name: "Supernova",
		internalSlug: "Mod_Supernova_C",
		imageUrl: "items/mods/supernova.png",
		id: "9rxk65",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Fires the compressed remains of a dying star. On contact, explodes for 150 -> 450 FIRE damage, and 350 -> 1050 BURNING damage over 10s to all targets within 4m.",
			"Striking the Supernova with Aphelion's primary fire spawns a massive shockwave which greatly increases the area of effect, total damage, and applies the initial BURNING amount.",
			"Generates 1 Primary Ammo on Fire.",
		],
		communityTags: [`AOE/Aura`, "Explosive Damage"],
		wikiUrl: `https://remnant2.wiki.gg/Aphelion`,
		linkedItems: {
			weapon: {
				name: "Aphelion",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Time Lapse",
		internalSlug: "Mod_TimeLapse_C",
		imageUrl: "items/mods/timelapse.png",
		id: "nn1q6k",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Creates a 6m blast which freezes all standard enemies for 7s.",
			"Dealing damage to frozen enemies immediately breaks the Time Lapse effect, applying SLOW for the remaining duration.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Time_Lapse`,
		location: {
			world: "Losomn",
			biome: "Streets of Losomn",
			injectable: "Hewdas Clock",
		},
	},
	{
		category: "MOD",
		name: "Tremor",
		internalSlug: "Mod_Tremor_C",
		imageUrl: "items/mods/tremor.png",
		id: "g6glpj",
		dlc: "BASE",
		description: [
			"Fires a projectile that cracks the ground and spawns shockwaves that deal 75 damage Within 9m for 6s. Shockwaves inflict 3x impact.",
		],
		communityTags: [`AOE/Aura`, `Explosive Damage`],
		wikiUrl: `https://remnant2.wiki.gg/Tremor`,
		location: { world: "Yaesha", dungeon: ["The Nameless Nest"] },
	},
	{
		category: "MOD",
		name: "Voidlight",
		internalSlug: "Mod_Voidlight_C",
		imageUrl: "items/mods/voidlight.png",
		id: "0ycjs9",
		dlc: "DLC3",
		description: [
			"Launches a projectile that releases Voidlight which remains dormant for 10s. While active, aborbs other damage sources.",
			"After expiration, explodes and fires up to 5 protectiles which home in on enemies within 15m. Projectiles deal 250 -> 750 Mod Damage split among enemies hit, dealing an additional 150 -> 450 per 100 Base Damage absorbed. Max 5x.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Voidlight`,
		location: { world: `N'Erud`, dungeon: ["Mucid Terrarium"] },
	},
	{
		category: "MOD",
		name: "Voltaic Rondure",
		internalSlug: "Mod_VoltaicRondure_C",
		imageUrl: "items/mods/voltaicrondure.png",
		id: "wjopwy",
		dlc: "BASE",
		searchableTags: ["Status Effect"],
		description: [
			"Launches a slow-moving orb that pulses every 0.5s, striking enemies within 6 for 20 -> 60 SHOCK damage and applying OVERLOADED, dealing 25 -> 75 SHOCK damage every 5s for 15s. The orb lasts 20s.",
			"Repeatedly damaging the orb overcharges it, increasing its radius. Overcharged orb deals 50% increased pulse damage.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Voltaic_Rondure`,
		location: { world: "Losomn", dungeon: ["The Great Sewers"] },
	},
	{
		category: "MOD",
		name: "Whirlwind",
		internalSlug: undefined,
		imageUrl: "items/mods/whirlwind.png",
		id: "2y6xm7",
		dlc: "BASE",
		description: [
			"Performing a Charged Melee Attack during a Neutral Backdash creates a Whirlwind of slashes which strike all enemies within 8m for 91 -> 273 damage.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Spectral_Blade`,
		linkedItems: {
			weapon: {
				name: "Spectral Blade",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Will of the Wisp",
		internalSlug: "Mod_WillOfTheWisp_C",
		imageUrl: "items/mods/willofthewisp.png",
		id: "9jwTXR",
		dlc: "BASE",
		description: [
			"Launches a Wisp that uniquely seeks to one anchored primary fire bolt location. Bolts anchored to enemies deal 100 -> 300 damage per Wisp. Bolts anchored to allies heal 10% Max Health per Wisp. Bolts are consumed when impacted by a Wisp.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Sorrow`,
		linkedItems: {
			weapon: {
				name: "Corrupted Sorrow",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Windfall",
		internalSlug: "Mod_Windfall_C",
		imageUrl: "items/mods/corrupted_windfall.png",
		id: "82v8kz",
		dlc: "BASE",
		description: [
			"Fires a ring of spinning blades which penetrates targets as it travels up to 20m before returning to the weapon. While traveling, activating Windfall again causes the projectile to spin in place for up to 3s. Activating it once more will recall it.",
			"Enemies struck by the Windfall obtain Tainted Blood for 3s causing all attacks from Deceit to trigger as Weakspot hits.",
		],
		wikiUrl: `https://remnant2.wiki.gg/Corrupted_Deceit`,
		linkedItems: {
			weapon: {
				name: "Corrupted Deceit",
			},
		},
		location: { world: "Any", dungeon: "Linked Item" },
	},
	{
		category: "MOD",
		name: "Witchfire",
		internalSlug: "Mod_Witchfire_C",
		imageUrl: "items/mods/witchfire.png",
		id: "ggf5qe",
		dlc: "BASE",
		description: [
			"Fires a highly volatile projectile that explodes to leave a line of flaming terrain. Deals 55 FIRE Damage per second, and applies BURNING, dealing 200 damage over 10s. Lasts 5s.",
		],
		communityTags: [`AOE/Aura`],
		wikiUrl: `https://remnant2.wiki.gg/Witchfire`,
		location: { world: "Losomn", dungeon: [`Cotton's Kiln`] },
	},
];

export { MODS, type Remnant2ModItem };
